  ...
  module Strategies
    class Oktaoauth < OmniAuth::Strategies::OAuth2
      ...
      def raw_info
        ...
        if options[:client_options] && options[:client_options][:user_info_url]
          user_info_url = options[:client_options][:user_info_url]
        else
          user_info_url = '/oauth2/' + options[:auth_server_id] + '/v1/userinfo'
        end
